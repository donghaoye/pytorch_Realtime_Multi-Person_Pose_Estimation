...
from pose_model import pose_model
...
def img2skeleton(real_img, skeleton_img):

    '''-----------------------------'''
    tic = time.time()
    oriImg = cv2.imread(real_img)  # B,G,R order
        ...
        if (nA != 0 and nB != 0):
            ...
            for i in range(nA):
                for j in range(nB):
                    ...
                    #z = (x / y) if y != 0 else 0
                    tmp = (0.5 * oriImg.shape[0] / norm) if norm != 0 else 0
                    score_with_dist_prior = sum(score_midpts) / len(score_midpts) + min(tmp - 1, 0)
    ...
    canvas = cv2.imread(real_img)  # B,G,R order
    canvas[...] = 0
    for i in range(18):
    ...
    cv2.imwrite(skeleton_img, canvas)


img2skeleton("sample_image/ski.jpg", "ttt.jpg")
